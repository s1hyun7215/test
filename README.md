...
현대인들은 만성 피로, 침침한 눈, 소화 불량 등 다양한 건강 이상 신호를 느끼지만,
이를 해결하기 위해 매번 병원을 가거나 인터넷 정보를 일일이 검색하는데 피로감을 느낍니다.

음식 레시피와 영양제 정보는 넘쳐나지만, **현재 나의 상태에 최적화된 식재료, 영양제와 
구매처를 한 번에 연결해 주는 서비스는 부족**하다는 점에 주목했습니다.

그래서 저희는 ‘**AI를 분석을 통한 개인 맞춤형 건강 큐레이션 서비스**’ 를 생각했습니다.

사용자가 일상적인 언어로 자신의 상태를 입력하면 AI(NLP)를 통해 핵심 키워드를 추출하여
즉각적으로 식재료와 영양제 추천을 제공합니다.

단순 정보 제공에 그치지 않고, 공공데이터 API와 쇼핑몰을 연동하여
**건강정보 입력 → 식재료 및 영양제 추천 → 식재료에 따른 레시피 제공 → 구매 연결까지 
이어지는 하나의 사용자 여정을 제공합니다.**
...
| 이름 | GitHub | 주요 작업 |
|------|--------|-----------|
| 전시현<br/>(팀장) | [@s1hyun7215](https://github.com) | 🔹 Redux 상태 관리 아키텍처 설계 및 구현 <br/>🔹 SPA 라우터 구성 <br/> 🔹 홈 페이지 구현 (증상 입력, Gemini AI 분석 연동) <br/> 🔹 결과 페이지 공통 UI 구현 |
